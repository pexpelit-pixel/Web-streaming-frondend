...
async function handleApi(req, env) {
  ...
  try {
    ...
    if (path === "/api/upload/file" && req.method === "POST") {
      ...
      const uploadRes = await fetch(uploadUrl, { method: "POST", body: doodForm });
    ...
    if (path === "/api/file/rename" && req.method === "POST") {
      const body = await req.json();
      if (!body.file_code || !body.title) throw new Error("file_code & title required");
    ...
    if (path === "/api/file/move" && req.method === "POST") {
      const body = await req.json();
      if (!body.file_code || !body.fld_id) throw new Error("file_code & fld_id required");
...
.pagination{display:flex;justify-content:center;gap:1rem;padding:2rem}
...
.uploader .form{display:flex;flex-direction:column;gap:1rem;max-width:600px;margin:2rem auto}
...
function videoCard(v) {
  return `
    <div class="video-card" onclick="location.href='/watch?file_code=${v.file_code}'">
      <img src="${v.single_img || v.splash_img || ''}" onerror="this.src='https://picsum.photos/200/130'">
      <div class="title">${escapeHtml(v.title)}</div>
      <div class="meta">${v.views} views • ${v.length}s</div>
    </div>`;
}
...
async function homePage(req, env) {
  ...
  const totalPages = parseInt(filesRes.result?.total_pages || "1");

  // Trending: ambil 10 video dengan views tertinggi dari halaman ini (untuk sederhana)
  const byViews = [...files].sort((a, b) => parseInt(b.views) - parseInt(a.views));
  const heroVideo = byViews[0];
  const trending = byViews.slice(0, 10);

  // Kategorikan video berdasarkan fld_id
  const folderMap = new Map();
  folders.forEach(f => folderMap.set(f.fld_id, f.name));
  const categorized = new Map();
  files.forEach(v => {
    const fid = v.fld_id || "0";
  ...
  const heroHtml = heroVideo ? `
    <div class="hero" style="background-image:url(${heroVideo.single_img || heroVideo.splash_img || ''})">
      <div class="hero-content">
        <h1>${escapeHtml(heroVideo.title)}</h1>
        <p>🔥 Trending #1 · ${heroVideo.views} views</p>
        <button onclick="location.href='/watch?file_code=${heroVideo.file_code}'">▶ Play</button>
        <button class="secondary" onclick="location.href='/watch?file_code=${heroVideo.file_code}'">ℹ Info</button>
      </div>
    </div>` : "";

  const trendingCards = trending.map(videoCard).join("");
  const paginationHtml = totalPages > 1 ? `<div class="pagination">
    ${page > 1 ? `<button onclick="location.href='?page=${page-1}&category=${category}'">‹ Prev</button>` : ""}
    <span>${page} / ${totalPages}</span>
    ${page < totalPages ? `<button onclick="location.href='?page=${page+1}&category=${category}'">Next ›</button>` : ""}
  </div>` : "";

  return new Response(baseHtml("XStreaming · Home", `
    ${heroHtml}
...
async function watchPage(req, env) {
  ...
  let meta = null;
  if (env.METADATA) {
    try { meta = await env.METADATA.get(`meta:${code}`, { type: "json" }); } catch {}
  }
  const title = meta?.title || video.title;
  const description = meta?.description || `Uploaded ${video.uploaded} • ${video.views} views`;

  const extraHead = meta?.description ? `<meta name="description" content="${escapeHtml(description)}">` : "";

  return new Response(baseHtml(title, `
    <div class="watch-container">
      <iframe src="https://dood.wf/e/${video.filecode}" allowfullscreen></iframe>
      <div class="watch-info">
        <h1>${escapeHtml(title)}</h1>
        <p>${escapeHtml(description)}</p>
        <p>👁 ${video.views} views • ⏱ ${video.length}s</p>
      </div>
    </div>
  `, extraHead), { headers: { "content-type": "text/html; charset=utf-8" } });
}
...
async function searchPage(req, env) {
  ...
  const grid = results.map(videoCard).join("") || "<p style='padding:2rem;color:var(--muted)'>Tidak ada hasil.</p>";
  return new Response(baseHtml(`Search: ${escapeHtml(q)}`, `
...
function uploaderPage() {
  return new Response(baseHtml("Upload Video", `
    <div class="uploader">
      <h2 style="text-align:center;margin:1rem 0">📤 Upload Video</h2>
      <div class="form">
        <h3>Via URL</h3>
        <input id="urlInput" placeholder="URL video langsung">
        <input id="urlTitle" placeholder="Judul">
        <input id="urlFolder" placeholder="Folder ID" value="0">
      ...
      <pre id="result" style="margin-top:2rem;background:rgba(255,255,255,.05);padding:1rem;border-radius:8px;max-height:300px;overflow:auto"></pre>
    </div>
    <script>
      ...
      function uploadFile() {
        ...
        xhr.onload = () => { try { show(JSON.parse(xhr.responseText)); } catch { show(xhr.responseText); } };
...
function embedPage(path) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Embed</title><style>body{margin:0;background:#000}iframe{width:100%;height:100vh;border:none}</style></head><body><iframe src="https://dood.wf/e/${code}" allowfullscreen></iframe></body></html>`;
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      // Public pages
      if (path === "/") {
        return homePage(req, env);
      }
      if (path === "/watch") {
        return watchPage(req, env);
      }
      if (path === "/search") {
        return searchPage(req, env);
      }
      if (path === "/uploader") {
        return uploaderPage();
      }
      if (path.startsWith("/embed/")) {
        return embedPage(path);
      }
      ...
      if (path.startsWith("/video/") && env.METADATA) {
        ...
        if (data?.file_code) {
          return Response.redirect(url.origin + `/watch?file_code=${data.file_code}`, 301);
        }
